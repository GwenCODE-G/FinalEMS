...
const SummaryModal = ({
  ...
  const fetchRealTimeSummary = useCallback(async () => {
    ...
    try {
      const response = await axios.get(
        `${apiBaseUrl}/api/rfid/summary/realtime/${monthlySummary.employee.employeeId}`,
        { timeout: 10000 }
      );
  ...
  const fetchAttendanceHistory = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${apiBaseUrl}/api/rfid/attendance?startDate=${startDate.toISOString().split('T')[0]}&endDate=${endDate.toISOString().split('T')[0]}&employeeId=${monthlySummary.employee.employeeId}`,
        { timeout: 10000 }
      );
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-6xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-blue-100 p-4 rounded-lg text-center border border-blue-200">
            ...
            <div className="text-2xl font-bold text-blue-600">{formatTotalHours(summaryData.totalHours)}</div>
            <div className="text-blue-800 text-sm">Total Hours</div>
            <div className="text-xs text-blue-600 mt-1">
              {summaryData.totalMinutes} total minutes
            </div>
          </div>
